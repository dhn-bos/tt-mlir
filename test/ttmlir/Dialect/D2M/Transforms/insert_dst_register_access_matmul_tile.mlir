// RUN: ttmlir-opt --ttcore-register-device --d2m-generic-tile-compute-loops --d2m-insert-dst-register-access="use-tile-matmul=true" --canonicalize -o %t %s
...
module {
  ...
  func.func @generic_matmul(
    ...
    ^compute0(%cb0: memref<3x3x!ttcore.tile<32x32, f16>, #l1_>, %cb1: memref<3x2x!ttcore.tile<32x32, f16>, #l1_>, %cb2: memref<3x2x!ttcore.tile<32x32, f16>, #l1_>):
      ...
      // CHECK-NEXT: affine.for %[[INIT_J:.*]] = 0 to 1 {

      // Check initialization: load from l1, store to dst
      // CHECK: %[[INIT_VAL:.*]] = affine.load %{{.*}}[%[[INIT_I]], %[[INIT_J]]] : memref<1x1x!ttcore.tile<32x32, f16>, strided<[2, 1], offset: ?>, #l1>
      ...
      // CHECK-NEXT: affine.for %[[J:.*]] = 0 to 1 {
      // CHECK-NEXT: affine.for %[[K:.*]] = 0 to 3 {

      // Check loads in computation loop: first two from l1, accumulator from dst
      // CHECK: %[[A_VAL:.*]] = affine.load %{{.*}}[%[[I]], %[[K]]] : memref<1x3x!ttcore.tile<32x32, f16>, strided<[3, 1], offset: ?>, #l1>
      // CHECK: %[[B_VAL:.*]] = affine.load %{{.*}}[%[[K]], %[[J]]] : memref<3x1x!ttcore.tile<32x32, f16>, strided<[2, 1], offset: ?>, #l1>
      // CHECK: %[[C_VAL:.*]] = affine.load %[[DST]][0, %[[I]], %[[J]]] : memref<8x1x1x!ttcore.tile<32x32, f16>, #dst>
      ...
      // CHECK: affine.store %[[MATMUL_RESULT]], %[[DST]][1, %[[I]], %[[J]]] : memref<8x1x1x!ttcore.tile<32x32, f16>, #dst>
      %c0 = arith.constant 0 : index
      ...
      // CHECK-NEXT: affine.for %[[WB_J:.*]] = 0 to 1 {

      // Check writeback: load from dst, store to l1
      // CHECK: %[[FINAL_VAL:.*]] = affine.load %[[DST]][1, %[[WB_I]], %[[WB_J]]] : memref<8x1x1x!ttcore.tile<32x32, f16>, #dst>
      // CHECK: affine.store %[[FINAL_VAL]], %{{.*}}[%[[WB_I]], %[[WB_J]]] : memref<1x1x!ttcore.tile<32x32, f16>, strided<[2, 1], offset: ?>, #l1>
    }
    return
  }
}
