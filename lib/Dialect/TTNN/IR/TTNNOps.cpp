...
::mlir::LogicalResult PagedUpdateCacheOp::verify() {
  ...
  int64_t numUsers = inputShape[1];
  int64_t numQueryHeads = inputShape[2];

  if (numQueryHeads % numCacheHeads != 0) {
    return emitOpError(
        "Expected number of query heads to be divisible by number of cache "
        "heads, got "
        "number of query heads = " +
        std::to_string(numQueryHeads) +
        ", number of cache heads = " + std::to_string(numCacheHeads));
  }

  if (inputShape[3] != headSize) {
    return emitOpError(
        "Expected input head size to be equal to cache head size, got "
        "input head size = " +
        std::to_string(inputShape[3]) +
        ", cache head size = " + std::to_string(headSize));
  }

  if (updateIndexShape[0] != numUsers) {
    return emitOpError("Expected update index tensor to have size equal to "
                       "number of users, got "
                       "update index tensor size = " +
                       std::to_string(updateIndexShape[0]) +
                       ", number of users = " + std::to_string(numUsers));
  }

  if (pageTableShape[0] != numUsers) {
    return emitOpError(
        "Expected page table tensor to have size equal to number of users, got "
        "page table tensor size = " +
