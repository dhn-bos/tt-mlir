...
template <>
struct EmitCTypeConverter<mlir::ArrayAttr> {
  static std::string convert(mlir::ArrayAttr arr) {
    if (!arr)
      return {};
    std::string buf;
    llvm::raw_string_ostream rso(buf);
    ...
    for (auto a : arr.getValue()) {
      if (first) {
        first = false;
        rso << EmitCTypeConverter<::ttnn::operations::unary::UnaryWithParam>::convert(a);
      } else {
        rso << ", " << EmitCTypeConverter<::ttnn::operations::unary::UnaryWithParam>::convert(a);
      }
    }
  ...
  static std::string convert(ttnn::Conv2dConfigAttr attr) {
    ...
    if (attr.getActivation()) {
      rso << (firstElement ? "" : ", ") << ".activation = "
          << EmitCTypeConverter<::ttnn::operations::unary::UnaryWithParam>::convert(attr.getActivation());
      firstElement = false;
    }
