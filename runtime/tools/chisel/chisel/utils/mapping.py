...
def custom_max_pool2d(*args, **kwargs):
    ...
    if (pt == pb) and (pl == pr):
        ...
        print(f"DEBUG max_pool2d - After pool: {out.dtype}")
    else:
        pad = [pl, pr, pt, pb]  # left, right, top, bottom
        print(f"DEBUG max_pool2d - Asymmetric padding case, before pad: {I.dtype}")
        I = torch.nn.functional.pad(I, pad)
        print(f"DEBUG max_pool2d - After pad, before pool: {I.dtype}")
        out = torch.nn.functional.max_pool2d(
            I,
            kernel_size=kernel_size,
            stride=stride,
